...
export default function Demo() {
  ...
  useEffect(() => {
    const treeEl = treeViewRef.current;
    if (!treeEl) return;
  ...
  return (
    <div className="flex gap-md p-md">
      ...
      <HvPanel classes={{ root: "bg-transparent" }}>
...
const getLeafIds = (node: TreeNodeData): string[] => {
...
const renderTreeItem = (
  ...
  return (
    <HvTreeItem
      ...
      label={
        <div
          className="flex items-center gap-xs w-full"
          role="button"
          tabIndex={-1}
          onClick={(e) => {
            e.stopPropagation();
            onCheckChange(node.id, node.label, !isChecked);
          }}
          onKeyDown={(e) => {
            if (e.key === "Enter")
              onCheckChange(node.id, node.label, !isChecked);
