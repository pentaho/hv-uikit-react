...
>
  {[...Array(7).keys()].map((i) => (
    <HvCarouselSlide
      key={i}
      src={`https://pentaho.github.io/assets/landscape${i}.jpg`}
      alt={`landscape ${i}`}
    />
  ))}
</Header>
...
const images = [...Array(7).keys()].map(
  (i) => `https://pentaho.github.io/assets/landscape${i}.jpg`,
);
```
...
<HvCarousel title="Travel Suggestions" carouselOptions={{ loop: false }}>
  {[...Array(7).keys()]
    .map((i) => `https://pentaho.github.io/assets/landscape${i}.jpg`)
    .map((src, i) => (
