...
You can customize the `Snackbar` by setting its position, transition direction, auto-hide duration and whether or not to show a progress bar.
...
- Use `showProgress` to display a progress bar indicating the remaining time before auto-hide.
      ...
      <div className="flex flex-col gap-md">
        <div className="w-full flex justify-between gap-xs items-end">
          ...
          <HvCheckBox
            ...
            onChange={(_, checked) => setShowProgress(checked)}
          />
        </div>
