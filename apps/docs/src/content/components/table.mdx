import { tableClasses as classes, HvTable } from "@pentaho/uikit-react-core";
...
```tsx
// create your own 👇 Table component, that receives external `data` & `column` configuration
export function MyTable({ data, columns }) {
  // ⚠️ alternatively, inline `data` & `columns`, which MUST be memoized.
