...
<Header
  ...
  controls={{
    ...
    spacing: { defaultValue: "loose" },
    maxVisible: { defaultValue: 4 },
    direction: { defaultValue: "row" },
    highlight: { defaultValue: false },
    toBack: { defaultValue: false },
  }}
>
  <HvAvatar alt="Ben" src="https://i.imgur.com/56Eeg1g.png" />
  <HvAvatar alt="Beatrice" />
  <HvAvatar alt="Wayne" src="https://i.imgur.com/ea22egF.png" />
  <HvAvatar alt="Clara" />
  <HvAvatar alt="Steve" />
  <HvAvatar alt="Sarah" />
  <HvAvatar alt="Cristina" />
</Header>
...
<HvAvatarGroup
  ...
  overflowComponent={(overflowCount) => (
    <HvTooltip title={`+${overflowCount} more users`}>
      <HvAvatar color="neutral" role="img">
        <HvIconContainer size="md">
          <div className="i-ph-dots-three" />
        </HvIconContainer>
      </HvAvatar>
    </HvTooltip>
  )}
