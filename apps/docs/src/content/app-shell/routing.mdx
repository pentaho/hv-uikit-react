...
Each _View_ is mapped to a URL path (defined by its `route` property) to enable navigation. The **App Shell** is responsible for loading the _View_ and rendering it at the correct moment and place. On the main panel, it does it according to the active location route.
...
Views support [conditions](./configuration#conditions) to control whether they are accessible. When all conditions for a view evaluate to `false`, attempting to navigate to that view's route will result in a 404 error.
...
> [!WARNING]
>
> Nested _Views_ are not supported by the file-based routing feature and must be declared manually in the **App Shell** configuration file. The automatic and manual definition of _Views_ will be merged.
...
For the **App Shell** the index route is the _View_ that is rendered when the URL matches the parent _View_'s route exactly. It is configured as a regular nested _View_ with `/` as its route:
