...
Views support [conditions](./configuration#conditions) to control whether a route is accessible. When all conditions for a view evaluate to `false`, attempting to navigate to that route will result in a 404 error.
...
When using conditions on views:

- Menu items targeting these routes will automatically inherit the view's conditions
- Users without proper access will see a 404 page when trying to access the route directly
- The route will not be accessible until all conditions return `true`
