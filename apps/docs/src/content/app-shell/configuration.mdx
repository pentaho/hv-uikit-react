...
- `themes`: Array of available themes. Can be one of the built-in UI Kit themes (`"next"`, `"pentaho"`), or a custom theme identified by the module.
...
```ts
const theming = {
  themes: ["next", "@hv-apps/my-app/tatooine.js"],
  theme: "tatooine",
  colorMode: "sand",
};
```
