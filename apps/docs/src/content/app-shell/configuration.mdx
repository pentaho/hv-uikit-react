...
System providers follow the [module structure](#module-structure) object API (important to note that they do not support conditions).
...
The usage of system providers should not be abused and well thought of since they will exist, regardless of conditional logic, and available to all application.
This may be useful when some context needs to be available before rendering any content or context required by conditions to evaluate properly.

**Key Differences from Regular Providers:**

- **Position in tree**: System providers wrap the entire App Shell (including the theme provider), while regular providers are nested deeper in the component tree
- **No conditions**: System providers are not affected by conditions
- **Context availability**: Context from system providers is available to conditions, regular providers, views, and all other modules

### `providers`

This prop defines the _Providers_ that will wrap all the _Views_ and _Shared Modules_. Unlike system providers, these can be conditionally loaded based on [conditions](#conditions). They follow the [module structure](#module-structure) object API.
...
Conditions are bundles that control whether certain elements (views, menu items, header actions, providers, or services) should be displayed or loaded in the final App Shell Model. They provide a powerful mechanism for conditional rendering based on dynamic runtime criteria.

#### How Conditions Work

A condition is a _Shared Module_ that exports a React hook as its default export. This hook must return an object with the following structure:
...
interface UseConditionResult {
  ...
  result: boolean; // The condition result
}
```
...
- **Multiple conditions**: When multiple conditions are present, ALL must return `true` for the element to be shown (AND logic)
- **Pending state**: Elements remain hidden while any condition is still pending (`isPending: true`)
- **Error handling**: If a condition returns an error, the element is treated as hidden
- **Menu inheritance**: Menu items automatically inherit conditions from their target view routes
...
const useIsAdmin = (): UseConditionResult => {
  const user = useCurrentUser(); // From a system provider context
...
#### Context from System Providers

Conditions can access context provided by [`systemProviders`](#systemproviders). This is particularly useful when conditions need to evaluate based on authentication state, user permissions, or global configuration:

```ts
// System provider gives context
const systemProviders = [{ bundle: "@hv-apps/auth/AuthProvider.js" }];

// Condition uses that context
const useIsAuthenticated = (): UseConditionResult => {
...
Services also support [`conditions`](#conditions) to control their availability dynamically:
...
- `colorMode`: The color mode of the theme (`"light"` or `"dark"`). Defaults to `theme`'s default default color mode.
