...
- **System Provider**: Module exporting a React Component that provides a React Context at the top of the component tree, wrapping the entire **App Shell**. System Providers are always loaded and cannot have conditions. They're particularly important for providing context to be used by other _Shared Modules_, specifically, (regular) _provider_ and _condition_ modules. They're configured in the **App Shell** configuration file in the [`systemProviders`](./configuration#systemproviders) array.
- **Provider**: Module exporting a React Component that provides a React Context to be used by other _Shared Modules_. Unlike System Providers, these can be conditionally loaded, but are not accessible to conditions. They're configured in the **App Shell** configuration file in the [`providers`](./configuration#providers) array.
- **Condition**: Module exporting a React Hook that returns an asynchronous boolean result to control which elements should be displayed or loaded. They're referenced in the **App Shell** configuration file using the [`conditions`](./configuration#conditions) property on various configuration objects.
