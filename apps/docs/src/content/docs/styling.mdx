...
```tsx
import { buttonClasses } from "@pentaho/uikit-react-core";
...
import { buttonClasses, theme } from "@pentaho/uikit-react-core";
...
**UI Kit** offers a utility classes plugin via a [UnoCSS](https://unocss.dev) plugin, `@pentaho/uikit-uno-preset`.
...
```sh
npm i -D unocss @pentaho/uikit-uno-preset
```
