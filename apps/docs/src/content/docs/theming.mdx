...
**UI Kit** ships with the **pentaho** theme by default, available in two color modes: **light** and **dark**:
...
- If `theme` is not provided, the default is `pentaho`.
...
- the static `theme` object from `@pentaho/uikit-react-core`
...
```tsx
const newTheme = mergeTheme(pentaho, {
  name: "myTheme",
  components: {
    HvAvatar: {
      variant: "square",
