...
export const HV_BUTTON_RULES: ValidationRule[] = [
  {
    ...
    check: (props) => {
      ...
      return {
        pass: isValid,
        reason: isValid ? undefined : `variant "${variant}" is not a valid HvButton variant`,
      };
    },
  ...
  },
  {
    ...
    check: (props) => {
      ...
      const isRawHex = typeof color === 'string' && /^#[0-9A-Fa-f]{6}|#[0-9A-Fa-f]{3}/.test(color);
      const isRawRgb =
        typeof color === 'string' && /^rgb(a?)\(/.test(color);
      const isStyleRaw = styleColor && (/^#[0-9A-Fa-f]{6}|#[0-9A-Fa-f]{3}/.test(styleColor) || /^rgb(a?)\(/.test(styleColor));

      return {
        pass: !(isRawHex || isRawRgb || isStyleRaw),
        reason: isRawHex || isRawRgb || isStyleRaw ? 'Raw color value instead of design token' : undefined,
      };
    },
    failureMessage: 'Raw color value instead of design token',
    suggestion: 'Use variant="negative" or import color token from @pentaho/uikit-styles',
    enforcement: 'error',
  },
  {
    componentName: 'HvButton',
    ...
    check: (props) => {
      ...
      if (selected === true && ariaPressed === undefined) {
        return {
          pass: false,
          reason: 'selected prop implies aria-pressed toggle semantics but aria-pressed is not set',
        };
      }

      return { pass: true };
    },
    failureMessage: '"selected" prop without aria-pressed implies incorrect toggle usage',
    suggestion: 'Add aria-pressed={isActive} and onClick handler for true toggles, or remove selected prop',
    enforcement: 'warn',
  },
  {
    componentName: 'HvButton',
...
export const HV_AVATAR_RULES: ValidationRule[] = [
  {
    ...
    check: (props) => {
      ...
      return {
        pass: isValid,
        reason: isValid ? undefined : `variant "${variant}" is not a valid HvAvatar variant`,
      };
    },
    failureMessage: 'Invalid variant name',
  ...
  },
  {
    ...
    check: (props) => {
      ...
      const style = (props.style as Record<string, any> | undefined)?.backgroundColor;

      if (backgroundColor || bgColor || style) {
        return {
          pass: false,
          reason: 'Custom backgroundColor should not be set when avatar will be in AvatarGroup',
        };
      }
    ...
    suggestion: 'Remove backgroundColor prop and let AvatarGroup control styling',
    enforcement: 'warn',
  },
  {
    componentName: 'HvAvatar',
